...
{
  "tool": "도구이름",
  "period": "today | this_week | last_week | recent_7_days | all",
...
period 선택 규칙:
- "오늘", "금일"이 있으면 "today"
- "이번주", "이번 주"가 있으면 "this_week"
- "저번주", "지난주", "전주", "저번 주", "지난 주"가 있으면 "last_week"
- "최근", "최근 7일", "요즘"이 있으면 "recent_7_days"
- 기간 표현이 전혀 없을 때만 "all"
...
3. 위 조건이 없고 "메뉴", "팔린", "팔리는" 등의 일반 표현이면 → "all_food"
    ...
    const finalAnswer = await callLLM(
      [
        {
          role: 'system',
          content: '너는 매장관리 데이터를 대표님이 이해하기 쉽게 설명하는 AI야. 숫자는 정확히 유지하고, 한국어로 짧고 명확하게 답변해.'
        },
        {
          role: 'user',
          content: `
