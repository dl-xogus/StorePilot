...
export async function POST(req) {
    try {
        ...
        console.log(maxmin.max); // 판매량 최고
        console.log(maxmin.min); // 판매량 최저
        ...
        // JSON 형식으로만 응답하도록 강제
        const prompt = `너는 JSON만 반환하는 API다.

다른 말 절대 하지 말고 JSON만 반환해.

다음은 매장의 판매량 통계입니다.
- 최고 판매량: ${JSON.stringify(maxmin.max)}
- 최저 판매량: ${JSON.stringify(maxmin.min)}

최고 판매 메뉴:
이름: ${JSON.stringify(maxmin.max?.name)}
판매량: ${JSON.stringify(maxmin.max?.sales)}

최저 판매 메뉴:
이름: ${JSON.stringify(maxmin.min?.name)}
판매량: ${JSON.stringify(maxmin.min?.sales)}
...
{"summary": "2-3줄 분석", "advice": "한 줄 조언"}`;
        ...
        return NextResponse.json({ ...maxmin, ...analysisResult });
    } catch (e) {
